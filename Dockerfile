# Buzz agent for the code-server environment: buzz-acp (from source) + Claude Code.
# Runs headless on cm3588, works in its OWN checkout under /workspace, routes
# Claude through code-server's pii-proxy. Never touches your Mac.
...
# Only buzz-acp — member management (add-member/roles) is a relay-side concern
# (the buzz-admin tool lives in the relay container), not the agent's.
RUN cargo build --release --locked -p buzz-acp --bin buzz-acp \
    && strip target/release/buzz-acp

FROM node:24-bookworm-slim
RUN apt-get update && apt-get install -y --no-install-recommends \
      ca-certificates git tini gosu && rm -rf /var/lib/apt/lists/*
